...
describe("Normal reproduces the day Orbit already builds", () => {
  ...
  it("places exactly the tasks the original greedy assignment placed", () => {
    // The original: bestFit per window, then drop a repeat so one task is
    // never offered in two windows. This is the parity guard -- if the mode
    // work changes what Normal does, demo beat 2 changes with it.
    const gaps = findGaps(blocks, profile, travel, NOW);
    const estimator = new Estimator();
    const original = new Map(gaps.map((g) => [g.id, bestFit(g, tasks, estimator)] as const));
    const seen = new Set<string>();
    for (const [id, task] of original) {
      if (task && seen.has(task.id)) original.set(id, undefined);
      if (task) seen.add(task.id);
    }

    const { picks, optional } = assignQuestsForMode(gaps, tasks, MODE_CONFIG.normal, estimator);
