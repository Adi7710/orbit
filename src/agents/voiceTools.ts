...
async function route(req: VoiceRequest): Promise<{ text: string; ok: boolean; data?: unknown }> {
  ...
  switch (req.tool) {
    ...
    case "get_coach": {
      ...
      if (facts.length === 0) {
        // Nothing from the weekly review yet. The completion history can still
        // answer -- that is what get_coach said before the learner existed, and
        // three weeks of sessions is not "I do not know enough about you".
        const profile = buildHabitProfile(s.habits);
