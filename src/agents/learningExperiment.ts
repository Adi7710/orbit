 ...
 */
export type Arm = "raw" | "existing" | "rules" | "nemotron";
...
export async function runExperiment(aspect: Aspect, arms: Arm[], opts: { weeks?: number; student?: SyntheticStudent } = {}): Promise<ExperimentResult> {
  ...
  for (const arm of arms) {
    ...
    for (let w = 1; w <= student.weeks; w++) {
      ...
      const planOf = (r: HabitRecord) => (arm === "raw" ? r.plannedMinutes : arm === "existing" ? planExisting!(r) : planMinutes(r.plannedMinutes, kindOf(r), learned));
    ...
    if (arm === "rules" || arm === "nemotron") {
      ...
      res.learned = kinds.map((k) => {
        const v = learned.multipliers[k]?.value ?? 1;
...
export function reportMarkdown(x: ExperimentResult): string {
  ...
  const label: Record<Arm, string> = { raw: "No learning (student's own estimate)", existing: "App today (per-course calibration)", rules: "Weekly learner, code decides", nemotron: "Weekly learner, Nemotron decides" };
